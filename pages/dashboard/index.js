...
export default function Dashboard() {
    ...
    return (
        <DashboardLayout title="Dashboard" menuItems={menuItems}>
            ...
            <div onClick={() => setIsModalShown(true)} className="w-1/4 border m-5 h-24 rounded-2xl p-5 relative text-center flex align-middle cursor-pointer">
                <p className="m-auto">Create New</p>
                <div className="m-auto">
                    <Image alt='' src="/icons/plus.svg" height={50} width={50} />
                </div>
            </div>
            <NewPageModal isShown={isModalShown} ownerUid={auth.userId} onClose={() => setIsModalShown(false)} onNewPage={newPage => setPages([...pages, newPage])} />
        </DashboardLayout>
    )
}